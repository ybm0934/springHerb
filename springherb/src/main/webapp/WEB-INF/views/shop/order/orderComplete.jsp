...
summary="주문 상품 목록에 관한 표로써, 상품명,가격, 수량, 금액 등의 정보를 제공합니다.">
	...
	<c:forEach var="map" items="${list }">
		<tr class="align_right">
			...
			<td>${map['QUANTITY'] }</td>
			<td>${map['QUANTITY']*map['SELLPRICE'] }</td>			
		</tr>	
	</c:forEach>
	<!-- 반복문 끝 -->	
	</tbody>
...
<div class="divForm">   
	...
	<p>
        ...
        <span>${ordersMap['SENDER'] }</span>        
    </p>
    <p>
        <span class="sp1">받으시는 분 </span>
        <span>${ordersMap['CUSTOMERNAME'] }</span>
    </p>
    
    <p>
        <span class="sp1">배송지 주소</span>
        <span>${ordersMap['ADDRESS'] } ${ordersMap['ADDRESSDETAIL'] }</span>
    </p>
    <p>
        <span class="sp1">휴대폰 번호</span>
        <span>${ordersMap['HP'] }</span>
    </p>        
    ...
    <p>
        ...
        <span><fmt:formatNumber value="${ordersMap['TOTALPRICE'] }" pattern="#,###"/> 원 </span>
    </p>
</div>
